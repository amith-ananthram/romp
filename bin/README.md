...
| `romp-tmux-env` | Bash | Sourced by `romp` before its first tmux call: resolves where the tmux server's socket lives (`TMUX_TMPDIR`, the operator's if set, else `$XDG_RUNTIME_DIR/romp`), the shell twin of `kernel/tmux_socket.py` and the manager's `tmuxTmpdir`; run directly it prints the directory. |
...
| `romp_tmux_socket.py` | `kernel/tmux_socket.py` | Where the tmux server's socket lives: one rule (an operator's `TMUX_TMPDIR` wins, else `$XDG_RUNTIME_DIR/romp` created 0700, else tmux's default), resolved into the kernel's environment before its first tmux call. |
