...
#     conversation the kernel resumed the CLI on.
# The registry learns a CLI's id only when the CLI's init message reaches the kernel
# (sdk_backend.py, SdkSession._on_message: the lastSid flip on the init's session_id), AFTER
# the CLI is up, and SdkBackend.spawn mints the reg with lastSid "" before the launch, so this
# hook can run ahead of that write: an EMPTY lastSid lets the start through. A /clear or a
# compaction rotates a LIVE CLI's id ahead of the same write, and neither is a child's first
# start, so those two sources pass on the payload's word. No id, no reg, or an unreadable
# reg: nothing to check against, so the hook does nothing (exit 0, never loud). The gate is
# kept verbatim in romp-postal-ensure.sh.
input="$(cat)"
