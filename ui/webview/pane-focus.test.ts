...
test("the wiring: the dismiss branch, the unfocused body, the composer, the restore on return, no adoption meanwhile", () => {
  ...
  assert.match(RENDER, /if \(vanishedId === msg\.id && heldHere\(msg\.id\)\) setActive\(msg\.id\);[^\n]*\n\s*const adopted = !activeId && !vanishedId && !wantActive && heldHere\(msg\.id\);/);   // …both gated on the column holding the session (the chat split, 2026-09-11)
  assert.match(fn("applyTabOrder"), /for \(const id of kernelOrder\) kernelListed\.add\(id\);[\s\S]{0,500}?const back = vanishedId \|\| wantActive;[^\n]*\n\s*colSets = readColSets\(\);[^\n]*\n\s*if \(back && order\.includes\(back\) && heldHere\(back\)\) setActive\(back\);/);   // …when this column holds it (the chat split)
