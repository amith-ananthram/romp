...
test("requestFullSession(id, why): every ask names its why, from the fixed vocabulary", () => {
  ...
  assert.ok(calls.length >= 7, "the gap, no-base ×3, skeleton-delta ×2, skeleton-click, prefetch and reattach sites");
  ...
  assert.equal(why("gap"), 2); assert.equal(why("nobase"), 3); assert.equal(why("skeleton-delta"), 2); assert.equal(why("reattach"), 1);   // gap ×2: the index tail's and the uuid tail's
